...
function buildHourlyForecast() {
  hourlyForecastData.forEach(function(hour) {
    ...
    $('#hourly-time').append(`
      <span class='box'><span class='time'>${displayHour}</span></span>
      `);
    $('#image').append(`
      <span class='box'><img src='./styles/svg/${icon}.svg'></span>
      `);
    $('#hourly-temperature').append(`
      <span class='box'><span class='temperature-small' id='hourly-temp'>${temperature}${temperatureScale}</span></span>
      `);
  });
};
